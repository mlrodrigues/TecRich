...
if ($senha != $csenha) {
    ...
    if ($logarray == $email) {
        ...
        $query = "INSERT INTO `tbl_usuario`(`cpf`, `nome`, `sobrenome`, `usuario`, `email`, `senha`, `tipo_cliente`, `telefone`, `celular`, `ativo`, `tbl_profissoes_id`) VALUES ('$cpf','$nome','$sobrenome','$usuario','$email','$senha','2','$telefone','$celular','0','$profissao')";
        $insert = mysqli_query($con, $query);

        if ($insert) {
			echo "<script language='javascript' type='text/javascript'>alert('Usuário cadastrado com sucesso!');window.location.href='index.php'</script>";
        } else {
			echo "<script language='javascript' type='text/javascript'>alert('Não foi possível cadastrar esse usuário');window.location.href='cadastro_pro.php'</script>";
        }
    }
}
?>